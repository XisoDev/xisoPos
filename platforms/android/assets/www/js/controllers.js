...
.controller('monthCtrl', function ($scope, $stateParams,$ionicModal, MultipleViewsManager, xSerial) {
    ...
    $scope.openMonth = function(){
        $scope.params = {};
        $scope.params.start_date = new Date();
        $scope.params.end_date = new Date(Date.parse($scope.params.start_date) + 30 * 1000 * 60 * 60 * 24);
        var start_date_ts = new Date($scope.params.start_date).getTime();
        var end_date_ts = new Date($scope.params.end_date).getTime();
